...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #FAF8F3; color: #1B2B4B;">
        <h1 style="color: #C9963A; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
        ...
        ${resultHtml}
        <hr style="border: 1px solid #FFFFFF; margin: 30px 0;" />
        <p style="font-size: 12px; color: #8A8F9A;">
